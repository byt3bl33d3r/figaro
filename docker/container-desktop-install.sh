...
# Start PulseAudio for speech synthesis support
sudoUserIf pulseaudio --start --daemonize=true --exit-idle-time=-1 2>&1 | sudoIf tee -a /tmp/pulseaudio.log > /dev/null
...
common_options="tigervncserver \${DISPLAY} -geometry \${screen_geometry} -depth \${screen_depth} -rfbport ${VNC_PORT} -dpi \${VNC_DPI:-96} -localhost 0 -desktop fluxbox -fg"
...
if [ -n "\${VNC_PASSWORD+x}" ]; then
    ...
    startInBackgroundIfNotRunning "Xtigervnc" sudoUserIf "\${common_options} -SecurityTypes None --I-KNOW-THIS-IS-INSECURE"
fi
